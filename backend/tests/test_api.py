...
def test_astro_automation_settings_endpoint_updates_runtime_service() -> None:
    ...
    with TestClient(app) as client:
        ...
        assert response.json()["allow_same_name_different_type"] is False
        ...
        saved = client.put(
            ...
            json={
                ...
                "allow_same_name_different_type": True,
            },
        )
        assert saved.status_code == 200
        assert saved.json()["alert_auto_create"] is True
        assert saved.json()["allow_same_name_different_type"] is True
        assert app.state.astro_alert_service.alert_auto_create_enabled is True
        assert app.state.astro_alert_service.allow_same_name_different_type is True

        reloaded = client.get("/api/settings/astro-automation")
        assert reloaded.status_code == 200
        assert reloaded.json()["alert_auto_create"] is True
        assert reloaded.json()["allow_same_name_different_type"] is True
