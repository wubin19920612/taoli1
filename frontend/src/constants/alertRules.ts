...
export const alertRuleGuide =
  "SF=现货买入 / 永续卖出，FF=永续买入 / 永续卖出，SS=现货买入 / 现货卖出。综合开仓=扣除手续费和滑点后的净开仓价差 + 资金费率差，优先使用预测资金费率。info=仅记录，warning=普通告警，critical=强提醒。连续命中：同一机会需要连续满足多少轮才触发。冷却秒数：同一机会触发后，多少秒内不重复发送。";
...
export const alertRuleFieldHelp = {
  ...
  min_fee_adjusted_open_pct: "扣除手续费、滑点后，再叠加资金费率差。正资金费率会加分，逆风资金费率会扣分。",
