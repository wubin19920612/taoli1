...
| 2026-09-24 08:12:20 | `sudo` 日志记录在 `~/wubin/taoli1` 执行 `docker compose build --pull`。本机监控代理在 08:12-08:15 记录内存使用率从约 70% 升至最高 98.759%，页面扫描指标最高 6,258,436；采样库每次采样的 `pgpgin` 最大增量为 3,362,856。08:12:25 起 Docker/BuildKit healthcheck 报连接错误，08:14 起 containerd 事件与容器健康检查超时。08:15 后代理样本中断，08:47:23 开始新一轮启动。 | **确认构建期间发生严重内存回收并伴随大量页读，随后服务失去响应**。本次内核日志没有 OOM kill、hung-task 或文件系统错误；不能称为“已确认 OOM 杀进程”。缺少冻结期间的进程 RSS 和云盘延迟，无法精确分解内存与 I/O 各自的影响。 |

### 云监控摘要的 00:20-00:45 时间轴冲突

用户提供的腾讯云自动摘要称 00:20 内存 98.588%、读盘 132,936 KB/s，00:40 读盘 133,052 KB/s，00:25-00:45 CPU/内存缺样，01:00 内存降至 46.38%；用户确认图表标为北京时间 UTC+8。服务器元数据中的实例名包含所给 Lighthouse 实例 ID，公网地址也匹配，因此以下本机采样来自同一实例。

监控代理本机 SQLite 采样库在北京时间 00:00-01:00 **每分钟都有 6 条样本**；其成功发送的原始指标在 00:20-00:45 共 156 条 CPU/内存样本，内存始终为 70.227%-71.515%，CPU 为 9.9%-34.0%，页面扫描最高 278。对应采样的 `pgpgin` 最大增量仅 424。00:20-00:45 的 journal 仍有 94 条记录，没有 OOM、Docker 构建或 Docker 服务错误；00:27 的 snapd 更新检查和 00:30 的云安全代理 cron 均不足以解释自动摘要所称读峰。云监控代理在该段持续记录 `nws send succ`，不能说它在午夜停止采集。

相反，摘要的 98.588% 与本机 **08:14:41 的 98.596%** 接近，摘要的 46.38% 与重启后 **09:04:08 的 46.384%** 接近；本机代理在 08:20、08:40 没有样本，08:48 后恢复。8 小时左右的时间错位与早晨构建、失联、重启的真实时间线高度吻合。尽管图表被描述为北京时间，自动摘要的时间标签或图表查询口径仍需用原始图表核实。摘要中的读盘峰也不能据当前资料确定持续时长或具体进程；本机页读计数确实在 08:13 大幅增加，但午夜没有对应变化。

“01:00 后没有监控”也不能推断主机从那时持续宕机：本机系统日志在 01:00-08:12 有 2,635 条，08:07-08:11 存在成功的 SSH 登录和数据库备份命令，监控代理采样持续至 08:15。控制台未显示的数据与本机原始记录之间的差异需要单独核对。
...
2. **9 月 24 日这一次的高可信机制**：生产主机内构建后，代理实测内存接近 99%，页扫描与页读计数剧增，Docker/BuildKit、containerd 和健康检查接连超时。无 swap 的主机很可能陷入内存回收与读盘竞争。没有内核 OOM kill 记录；无法证明发生了 OOM 杀进程，也不能量化云盘延迟或确认具体读盘进程。
...
3. 取得云监控原始图表，核对时区、采样粒度、聚合口径和实例维度，尤其是自动摘要中 00:20/00:40/01:00 的标签为何与本机 08:14/08:40/09:04 的记录高度吻合；再查看 9 月 13 日 00:35-00:45、9 月 23 日 12:00-12:10、9 月 24 日 08:10-08:47 的云盘延迟/队列与实例事件。启用分钟级外部监控与告警；下次异常优先保留冻结时指标，再判断是否需要调整 BuildKit 并行度或服务资源边界。
