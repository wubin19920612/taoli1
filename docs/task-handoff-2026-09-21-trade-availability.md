...
## 2026-09-22 页面排版与逐链充提展示

### 本轮目标与结果

- 移除交易可用性区域原有 `2515px` 横向宽表，改为每个精确原始市场一条自适应记录；桌面和手机均不再依赖横向拖动。
- 永续动作合并为“普通开仓”和“Reduce Only 平仓”两个动作组，分别显示“开多”“开空”“平空 · Buy”“平多 · Sell”。
- 现货只显示“买入”和“卖出”，不渲染做空、Reduce Only 或“不适用”。
- 公开市场状态、账户级核验情况和真实订单错误继续分开显示；异常或未知市场直接显示精确市场的恢复订阅按钮。
- 买一、卖一、1% 双边深度、24h 成交额、资金费率及周期、预估资金、Maker/Taker、手续费是否计入、价格倍率、数量乘数、标记/指数价、盘口来源及三类更新时间改为可换行指标带。
- 现货充提从市场宽表拆为独立区域，直接遍历 `spot_transfer.networks`；同一资产有多条网络时，每条网络独立一行，分别展示充币和提币状态、来源与更新时间。
- 单条网络不再使用聚合文案“全开”或“部分开放”：`true` 显示“开启”，`false` 显示“暂停”，`null` 显示“未知”。只有公开接口将来明确返回账户、地区、额度或通道限制时才可显示“受限”，当前布尔逐链接口不推测限制。
- 公开接口未返回网络列表时显示“未返回链列表 / 未知”，不会把缺失数据误报为开启或暂停。
...
- 页面专项：`19 passed`。
- 前端生产构建：通过。
- 前端全量：`170 passed, 1 failed`。唯一失败仍为既有 Settings 测试查找旧文案“实盘灰度”，本轮未修改 Settings 模块。
- `git diff --check`：通过。
- 使用真实 BTC 公开数据完成浏览器检查：14 个原始市场、6 个现货市场均展示；当次逐链数据包括 Binance 5 条、Gate 2 条、Bitget 3 条，逐条显示充币和提币开关。
- `1440px`：页面、交易区和充提区的 `scrollWidth` 均等于 `clientWidth`（`1440/1440`、`1190/1190`、`1190/1190`）。
...
## 2026-09-22 账户条件文案与指标可读性优化

### 行为与排版

- Reduce Only 的 `conditional` 不再统一显示含义模糊的“账户有条件”：平空显示“需空仓”，平多显示“需多仓”；悬浮说明明确系统没有读取账户实际仓位，仍需对应持仓、正确方向和不超过持仓的数量。
- 账户级公开诊断从“账户 未核验”改为“账户未接入”，继续与公开市场状态和真实订单错误分开，不暗示已经完成账户检查。
- 永续动作区压缩为“普通开仓”和“Reduce Only”两条紧凑动作组；现货仍只显示买入、卖出。
- 行情指标按盘口、成交与费用、合约信息、数据源与时效分组。标签和值上下分层，数值使用 12px 高对比颜色；买一为绿色，卖一和负资金费率为红色，其余关键数值使用深色。

### 测试与视觉验证

- 页面专项：`19 passed`。
- TypeScript 与 Vite 生产构建：通过。
- 前端全量：`174 passed, 1 failed`。唯一失败仍为既有 Settings 测试查找旧文案“实盘灰度”，本轮未修改 Settings 模块。
- `git diff --check`：通过。
- 使用真实 BNC 数据完成本地和生产浏览器检查；生产返回 4 个精确市场，页面可见“账户未接入”“需空仓”“需多仓”。
- 桌面 `1440px`：页面、交易状态区、市场行、动作区和指标区均无横向溢出，分别为 `1440/1440`、`1190/1190`、`1190/1190`、`599/599`、`1190/1190`。
- 手机 `390px`：对应宽度为 `390/390`、`372/372`、`372/372`、`348/348`、`372/372`，没有横向拖动或文字遮挡。
- 本轮截图保存在未跟踪的 `output/trade-availability-compact-*.png`，不提交仓库。

### 生产交付

- 功能提交：`5ec8ed7 refactor: clarify trade status metrics`，已推送 `origin/codex/frontend-localization-polish`。
- 部署前数据库备份：`backups/radar-20260922T042406Z.db`，大小 `598925312` 字节，SHA-256 `8decce1a35366e71f0c3f3c3fb78bb7a464ed7ec1ff7bb9c9631d28a68279662`。
- 源库、容器内在线备份和主机备份的 `PRAGMA quick_check` 均为 `ok`；主机备份确认后删除了卷内临时副本，线上 `/data/radar.db` 保持存在。
- 同一分支在部署时已包含另一个账户连接任务，因此服务器通过 `git pull --ff-only` 更新到包含本功能提交的 `c1a8c18`，再执行 `docker compose build --pull` 和 `docker compose up -d --remove-orphans`；没有执行 `down -v`。
- 前后端容器均为 `healthy`；`/api/health` 返回 `status=ok`，八家交易所采集器全部为 `healthy`。
- 生产页面 HTTP 200，桌面和手机 DOM、计算样式及局部截图验证通过。本轮没有发送订单、点击订阅或人为触发飞书恢复通知。
